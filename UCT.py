...
class UCBState:
    def __init__(self, positions=[[0] * 6 for _ in range(7)], exploreParam=.1):
    ...
    def update(self, action, depth, depthBudget):
        ...
        if depth > 1:
            for sample in range(depthBudget[depth]):
                childState.UCBTree(depth - 1, depthBudget)
        else:
            childState.UCBTree(0, depthBudget)
    ...
    def UCBTree(self, depth, depthBudget):
