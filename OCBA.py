...
class OCBAState:
    def __init__(self, positions=[[0] * 6 for _ in range(7)], initProbeBudget=4):
    ...
    def update(self, action, depth, depthBudget): # depth budget gives budget at each depth level
        positions = self.sampleY(action)
        # divide actions in two for now
        childState = OCBAState(positions=positions, initProbeBudget = self.initProbeBudget)
        if depth > 1:
            for sample in range(depthBudget[depth]):
                childState.OCBATree(depth - 1, depthBudget)
        else:
            childState.OCBATree(0, depthBudget)
    ...
    def OCBATree(self, depth, depthBudget):  # returns vHat
        ...
        self.update(starvingAction, depth, depthBudget)
        self.updateVHat()
